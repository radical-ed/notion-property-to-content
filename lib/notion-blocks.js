...
// Notion's File Upload API predates the installed @notionhq/client version,
// so it's called directly. Three steps: create a file_upload object, POST
// the bytes to its upload_url, then reference the id in a block. See
// docs/workflowy-to-notion.md for the reference this was validated against.
export async function uploadImageToNotion (token, buffer, filename, contentType) {
  ...
  if (!createResponse.ok) {
    throw new Error(`Failed to create Notion file upload (${createResponse.status}): ${await createResponse.text()}`)
  }
  const { id, upload_url: uploadUrl } = await createResponse.json()
  ...
  if (!sendResponse.ok) {
    throw new Error(`Failed to upload file bytes to Notion (${sendResponse.status}): ${await sendResponse.text()}`)
  }
